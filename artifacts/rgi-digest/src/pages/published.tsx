import { useListDigestArticles, useRejectDigestArticle, DigestArticle } from "@workspace/api-client-react";
              ...
              <div className="flex flex-col items-end gap-0.5">
                ...
                {article.publishedAt && (
                  <span className="text-[11px] text-muted-foreground/60">
                    <span className="font-medium">Approved:</span> {format(safeDate(article.publishedAt), "MMMM d, yyyy")} — {format(safeDate(article.publishedAt), "HH:mm")}
                  </span>
                )}
              </div>
            </div>
...
export default function Published() {
  ...
  const { mutate: rejectArticle, isPending: isDeleting } = useRejectDigestArticle({
    mutation: {
      onSuccess: () => {
        refreshWorkflowData();
  ...
  const dailyBriefs = safeArticles.filter((a) => a.articleType === "daily_brief");
  const topicArticles = safeArticles.filter((a) => a.articleType === "topic_article");
                ...
                {dailyBriefs.map((article) => (
                    ...
                    >
                          ...
                          <div className="space-y-1.5 flex-1 min-w-0">
                            <div className="flex items-center gap-2 flex-wrap">
                              ...
                              <div className="ml-auto flex items-center gap-3">
                                <div className="flex flex-col items-end gap-0.5">
                                  <span className="text-[11px] text-muted-foreground">
                                    <span className="font-medium">Generated:</span> {format(safeDate(article.createdAt, new Date()), "MMM d, yyyy")} — {format(safeDate(article.createdAt, new Date()), "HH:mm")}
                                    {" "}<span className="text-muted-foreground/50">({formatDistanceToNow(safeDate(article.createdAt, new Date()), { addSuffix: true })})</span>
                                  </span>
                                  {article.publishedAt && (
                                    <span className="text-[10px] text-muted-foreground/55">
                                      <span className="font-medium">Approved:</span> {format(safeDate(article.publishedAt), "MMM d, yyyy")} — {format(safeDate(article.publishedAt), "HH:mm")}
                                    </span>
                                  )}
                                </div>
                                <Button
                      ...
                      <CardContent className="space-y-3">
                        {asStringArray(article.executiveSummary).length > 0 ? (
                          <ul className="space-y-1">
                            {asStringArray(article.executiveSummary).slice(0, 3).map((bullet, i) => (
                              <li key={i} className="flex items-start gap-2 text-xs text-muted-foreground">
                                <span className="text-primary/60 font-bold mt-0.5 shrink-0">•</span>
                                {bullet}
                              </li>
                            ))}
                            {asStringArray(article.executiveSummary).length > 3 && (
                              <li className="text-xs text-primary/60 pl-4">
                                +{asStringArray(article.executiveSummary).length - 3} more points
                              </li>
                            )}
                          </ul>
                        ) : (
                      ...
                      <CardContent className="space-y-3">
                        ...
                        {asStringArray(article.topicTags).length > 0 && (
                          <div className="flex flex-wrap gap-1.5">
                            {asStringArray(article.topicTags).slice(0, 5).map((tag) => (
                              <Badge key={tag} variant="secondary" className="text-xs font-normal">{tag}</Badge>
                            ))}
                          </div>
                        )}
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              <span className="font-medium text-foreground">"{asString(deleteTarget?.headline, "this article")}"</span> will be moved to the Rejected section. You can permanently delete it from there.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              disabled={isDeleting}
              onClick={() => deleteTarget && rejectArticle({ id: deleteTarget.id, data: { reason: "Rejected from Published Archive" } })}
            >
