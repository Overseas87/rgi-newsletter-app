...
export default function Feed() {
  ...
  const counts = useMemo(() => {
    const a = Array.isArray(safeArticles) ? safeArticles : [];
  ...
  const filtered = useMemo(() => {
    let result = Array.isArray(safeArticles) ? safeArticles : [];
    ...
    if (search.trim()) {
      ...
      result = result.filter(
        (a) =>
          ...
          asStringArray(a.topicTags).some((t) => t.toLowerCase().includes(q))
      );
    }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            {safeArticles.length} signals across all sources
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          <p className="text-xs text-muted-foreground">
            Showing {filtered.length} of {safeArticles.length} articles
