...
function SourceRow({ source }: { source: Source }) {
  ...
  return (
    <Card data-testid={`source-row-${source.id}`}>
          ...
          <div className="flex items-center gap-4">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-0.5">
                ...
                <Badge variant="outline" className={`text-xs ${TIER_COLORS[source.tier]}`}>
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage RSS feeds and news sources by tier.</p>
        </div>
      ...
      {showAdd && (
        <Card>
          <CardContent className="p-5 space-y-4">
            ...
            <div>
              <Label className="text-xs mb-1 block">RSS/Atom Feed URL</Label>
              <Input
                value={newUrl}
          ...
          {grouped.map(({ tier, sources: tierSources }) => (
            <div key={tier}>
              <h2 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground mb-3">
                {TIER_LABELS[tier]}
                <span className="ml-2 font-normal">({tierSources.length})</span>
              </h2>
