...
type SourceWithWeight = Source & { weight?: number | null };
...
function computeCredibility(source: SourceWithWeight): { score: number; credibility: number; explanation: string; shortLabel: string } {
...
function SourceRow({ source }: { source: SourceWithWeight }) {
  ...
  const handleSave = () => {
    update.mutate(
      { id: source.id, data: { name, url, tier: Number(tier) as 1 | 2 | 3, weight: editWeight } as any },
      {
...
export default function Sources() {
  ...
  const queryClient = useQueryClient();
  const safeSources = asArray<SourceWithWeight>(sources);
  ...
  const typeCounts = Object.keys(TYPE_META).reduce((acc, t) => {
    acc[t] = safeSources.filter((s: SourceWithWeight) => s.type === t).length;
    return acc;
  }, {} as Record<string, number>);

  const grouped = [1, 2, 3].map((tier) => ({
    tier,
    sources: safeSources.filter((s: SourceWithWeight) => s.tier === tier),
  }));

  const avgByTier = [1, 2, 3].reduce((acc, tier) => {
    const t = safeSources.filter((s: SourceWithWeight) => s.tier === tier);
    acc[tier] = t.length > 0
      ? Math.round(t.reduce((s, src) => s + computeCredibility(src).credibility, 0) / t.length * 10) / 10
      : 0;
    return acc;
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">
            {safeSources.filter((s: SourceWithWeight) => s.isActive).length} active sources across {grouped.filter((g) => g.sources.length > 0).length} tiers
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="py-16 text-center text-muted-foreground">
          <p className="font-medium mb-1">Failed to load sources</p>
          ...
          {grouped.map(({ tier, sources: tierSources }) => (
            <div key={tier}>
              ...
              {tierSources.length === 0 ? (
                ...
                <div className="space-y-2">
                  {tierSources.map((source: SourceWithWeight) => (
