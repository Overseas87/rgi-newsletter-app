...
import { asArray, asNumber, asString, asStringArray, safeLines } from "@/lib/arrays";
...
export function GenerateModal({ open, onOpenChange, initialMode = "topic_article" }: GenerateModalProps) {
  ...
  const refreshWorkflowData = () => {
    queryClient.invalidateQueries({ predicate: (query) => query.queryKey[0] === "/api/digest" });
  ...
  const pollJob = async (jobId: string): Promise<Record<string, unknown>> => {
    ...
    while (Date.now() - started < 180_000) {
      const res = await fetch(`${base}/api/jobs/${jobId}`, { headers: { accept: "application/json" } });
      if (!res.ok) throw new Error("Job status unavailable");
      const job = (await res.json().catch(() => ({}))) as JobResponse;
      if (typeof job.progress === "number") {
        setLoadingStage(Math.min(LOADING_STAGES.length - 1, Math.floor(job.progress / 25)));
      }
    ...
    throw new Error("Generation is still running. Check Pending Review in a moment.");
  };

  const articleFromPayload = (data: Record<string, unknown>): GeneratedArticle => ({
    id: asNumber(data.id),
    ...
    implificationsForLeaders: asStringArray(data.implificationsForLeaders),
    whatMostAreMissing: typeof data.whatMostAreMissing === "string" ? data.whatMostAreMissing : null,
    mechanism: asStringArray(data.mechanism),
    constraintsAndRisks: asStringArray(data.constraintsAndRisks),
    whatToWatch: asStringArray(data.whatToWatch),
    topicTags: asStringArray(data.topicTags),
    discipline: asString(data.discipline, "Multiple"),
  ...
  const fetchCandidates = async () => {
    ...
    try {
      ...
      const data = asArray<CandidateArticle>(await res.json().catch(() => []));
      const topics = Array.from(selectedTopics);
      const matching = data
        .filter((a) => asStringArray(a.topicTags).some((t) => topics.includes(t)))
  ...
  const handleSelectAll = () => {
    if (selectedArticleIds.size === candidates.length) {
      setSelectedArticleIds(new Set());
        ...
        setSelectedArticleIds(new Set(candidates.map((a) => asNumber(a.id)).filter((id) => id > 0)));
    }
  };
  ...
  const handleGenerateDailyBrief = async () => {
    ...
    try {
      console.log("[daily-brief-trace] frontend action: Generate Daily Brief clicked");
      console.log("[daily-brief-trace] endpoint: /api/digest/daily-brief-job");
      console.log("[daily-brief-trace] requested behavior: fresh generation, no cache reuse");
      const res = await fetch(`${base}/api/digest/daily-brief-job`, {
        method: "POST",
      ...
      if (modalOpenRef.current) {
        ...
        toast({
          ...
          description: `"${asString(data.headline, "Brief").slice(0, 70)}…" has been added to Pending Review.`,
        });
  ...
  const handleGenerateFromSelection = async () => {
    ...
    try {
      const res = await fetch(`${base}/api/digest/generate-job`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          articleIds: ids,
  ...
  const handleRefine = async () => {
    ...
    try {
      ...
      const refined = await res.json().catch(() => ({}));
      setGeneratedArticle((prev) => prev ? {
        ...prev,
        headline: asString(refined.headline, prev.headline),
        body: asString(refined.body, prev.body),
        executiveSummary: asStringArray(refined.executiveSummary).length > 0 ? asStringArray(refined.executiveSummary) : prev.executiveSummary,
        rgiTake: asString(refined.rgiTake, prev.rgiTake),
        keyTakeaways: asStringArray(refined.keyTakeaways).length > 0 ? asStringArray(refined.keyTakeaways) : prev.keyTakeaways,
        implificationsForLeaders: asStringArray(refined.implificationsForLeaders).length > 0 ? asStringArray(refined.implificationsForLeaders) : prev.implificationsForLeaders,
        whatMostAreMissing: refined.whatMostAreMissing ?? prev.whatMostAreMissing,
        mechanism: asStringArray(refined.mechanism).length > 0 ? asStringArray(refined.mechanism) : prev.mechanism,
        constraintsAndRisks: asStringArray(refined.constraintsAndRisks).length > 0 ? asStringArray(refined.constraintsAndRisks) : prev.constraintsAndRisks,
        whatToWatch: asStringArray(refined.whatToWatch).length > 0 ? asStringArray(refined.whatToWatch) : prev.whatToWatch,
      } : prev);
  ...
  const handleSaveAsDraft = () => {
    if (generatedArticle?.id) {
      queryClient.setQueryData(["/api/digest", { status: "pending_review" }], (current: unknown) => {
        ...
        if (existing.some((item: any) => item?.id === generatedArticle.id)) return existing;
        return [{ ...generatedArticle, status: "pending_review" }, ...existing];
      });
    }
    refreshWorkflowData();
    toast({
      title: "Saved to Pending Review",
      description: `"${asString(generatedArticle?.headline, "Brief").slice(0, 60)}..." is ready for your review.`,
    });
    handleClose();
    navigate("/review");
  };
  ...
  const handleSpeak = () => {
    ...
    const text = `${asString(generatedArticle.headline)}. ${asString(generatedArticle.body)}`;
          ...
          <div className="overflow-y-auto flex-1 px-4 py-3 space-y-2">
            {candidates.length === 0 ? (
              ...
              candidates.map((article) => {
                const articleId = asNumber(article.id);
                const isSelected = selectedArticleIds.has(articleId);
                const topScore = asNumber(article.relevancyScore) >= 8.5;
                return (
                  <div
                    key={articleId}
                  ...
                  >
                    <Checkbox
                      checked={isSelected}
                      onCheckedChange={() => toggleArticle(articleId)}
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded shrink-0 ${
                          ...
                          {asNumber(article.relevancyScore).toFixed(1)}
                        </span>
                      </div>
                      <div className="flex items-center gap-2 mt-1 flex-wrap">
                        <span className="text-[10px] text-muted-foreground font-medium">{asString(article.sourceName, "Unknown source")}</span>
                        {asStringArray(article.topicTags).slice(0, 3).map((t) => (
          ...
          <DialogHeader className="px-6 pt-5 pb-3 border-b border-border shrink-0">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                {generatedArticle.generationMode === "fallback" && (
                  <span
                    className="inline-flex items-center gap-1 text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-slate-50 text-slate-700 border border-slate-200"
                    title={generatedArticle.fallbackReason || "Generated with source-backed fallback synthesis"}
                  >
                    Fallback mode
                  </span>
                )}
          ...
          <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
            <div className="space-y-4">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {asStringArray(generatedArticle.topicTags).slice(0, 4).map((t) => (
                  <Badge key={t} variant="outline" className="text-[10px] border-primary/30 text-primary">{t}</Badge>
                ))}
              </div>
