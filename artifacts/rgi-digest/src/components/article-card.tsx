...
function AuthenticityBadge({ score }: { score?: number | null }) {
  ...
  if (safeScore >= 8) {
    ...
    label = `Auth ${safeScore.toFixed(1)}`;
  } else if (safeScore >= 5.5) {
...
export function ArticleCard({ article, selectable, selected, onSelect, onTopicClick, moderationActions = [] }: ArticleCardProps) {
  ...
  const publishedDate = publishedAt ?? scrapedAt;
  const showExplain = score >= 6.5;
  const articleUrl = asString(article.url).trim();
  const hasUrl = articleUrl.length > 0;
  ...
  return (
    <Card
      ...
      } ${Boolean(article.isPrimarySignal) ? "border-violet-500/40 bg-violet-500/[0.02]" : Boolean(article.isEmergingSignal) ? "border-amber-500/30" : ""}`}
      data-testid={`article-card-${asNumber(article.id)}`}
    >
          ...
          <div className="flex-1 min-w-0 space-y-2">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="text-xs text-muted-foreground flex flex-col gap-0.5">
                {article.publishedAt ? (
                  <span title={format(publishedDate, "MMMM d, yyyy 'at' h:mm a")}>
                    <span className="text-muted-foreground/50 font-medium">Source published: </span>
                    {format(publishedDate, "MMM d, yyyy")}
                    <span className="text-muted-foreground/40"> — </span>
                    {format(publishedDate, "h:mm a")}
                    <span className="text-muted-foreground/40"> · </span>
                    {formatDistanceToNow(publishedDate, { addSuffix: true })}
                  </span>
                ) : null}
                <span title={format(scrapedAt, "MMMM d, yyyy 'at' h:mm a")}>
                  <span className="text-muted-foreground/50 font-medium">RGI ingested: </span>
                  {format(scrapedAt, "MMM d, yyyy")}
                  <span className="text-muted-foreground/40"> — </span>
                  {format(scrapedAt, "h:mm a")}
                  {!article.publishedAt && (
                    <>
                      <span className="text-muted-foreground/40"> · </span>
                      {formatDistanceToNow(scrapedAt, { addSuffix: true })}
                    </>
                  )}
                </span>
              </span>
              <div className="ml-auto flex items-center gap-1.5 flex-wrap justify-end">
                {Boolean(article.isPrimarySignal) && (
            ...
            <CardTitle className="text-base leading-snug font-semibold">
              {hasUrl ? (
                <a
                  ...
                  data-testid={`link-article-${asNumber(article.id)}`}
                >
                ...
                <span className="text-muted-foreground/70 flex items-start gap-1.5" data-testid={`link-article-${asNumber(article.id)}`}>
                  {asString(article.headline, "Untitled article")}
        ...
        <CardContent className="px-4 pb-2 space-y-1.5">
          {(article.teaserSummary || article.content) && (
            <p className="text-sm text-muted-foreground leading-relaxed line-clamp-2 ml-7">
              {asString(article.teaserSummary || article.content).substring(0, 200)}
            </p>
          )}
          {article.viewpoint && (
      ...
      {showExplain && (
        <div className="px-4 pb-3">
          <RgiExplanationPanel articleId={asNumber(article.id)} discipline={article.disciplineAlignment} />
        </div>
      )}
    </Card>
  );
