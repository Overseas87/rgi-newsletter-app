...
router.get("/dashboard/summary", async (req, res): Promise<void> => {
  ...
  if (useSupabaseData()) {
    try {
      const [articles, digestArticles, sources] = await Promise.all([
        listSupabaseArticles({ limit: 500, sortBy: "time" }),
        listSupabaseDigests({ limit: 200 }),
        listSupabaseSources(),
      ]);

      const todayArticles = articles.filter((a) => new Date(a.scrapedAt).getTime() >= today.getTime());
      const contentWindow = todayArticles.length >= 15 ? today : sevenDaysAgo;
      const windowArticles = articles.filter((a) => new Date(a.scrapedAt).getTime() >= contentWindow.getTime());
      const topArticles = windowArticles
        .filter((a) => Number(a.relevancyScore ?? 0) >= 6)
        .sort((a, b) => Number(b.relevancyScore ?? 0) - Number(a.relevancyScore ?? 0))
        .slice(0, 10);
      ...
      for (const article of windowArticles) {
        ...
        if (Number(article.relevancyScore ?? 0) < 6) continue;
      ...
      req.log.error({ err: e }, "Failed to build Supabase dashboard summary");
      res.json({
  ...
  const signalClusters = buildSignalClusters(topCandidates as any, 12);
...
router.get("/topics", async (req, res): Promise<void> => {
  try {
    const articles = useSupabaseData()
      ? await listSupabaseArticles({ limit: 500 })
      : await db.select().from(articlesTable).orderBy(desc(articlesTable.scrapedAt)).limit(500);

    const tagData: Record<string, { count: number; totalScore: number; hasEmergingSignal: boolean }> = {};
    for (const article of articles) {
      if ((article.relevancyScore ?? 0) < 6) continue;
      for (const tag of Array.isArray(article.topicTags) ? article.topicTags : []) {
        if (!tagData[tag]) tagData[tag] = { count: 0, totalScore: 0, hasEmergingSignal: false };
        tagData[tag].count++;
...
router.get("/dashboard/settings", async (req, res): Promise<void> => {
  if (useSupabaseData()) {
    res.json(await getSupabaseSettings());
    return;
  }
...
router.patch("/dashboard/settings", async (req, res): Promise<void> => {
  ...
  if (useSupabaseData()) {
    res.json(await upsertSupabaseSettings(body.data));
    return;
  }
